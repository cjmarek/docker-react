...
without mapping :/app, then that tells docker not to try to find node_modules on the dockerfile source.
...
* by creating a docker-compose file, we no longer need to manually fire up a container from an image as we did above,
docker-compose up will kick off a build and then run the container, and not need to know about volumes and ports or image ID's, with this instead. . . 
...
* to run a specific command inside of a container, by using keyword 'exec', that will OVERRIDE the dockerfile commands of CMD ["npm", "run", "start"] in the container


docker exec -it [container id] npm run testdocker exec -it ad2cfc391b76 npm run test
